...

{
  ...
  "metaRoutingRuleId": "cmo8a4zla00xnmw20myzwv5yl",
  "consentProof": {
...
| `metaRoutingRuleId` | string | recommandé | **Identifiant de la campagne / règle de routage** à appliquer au lead. Détermine vers quel pool de courtiers le lead est distribué. Si omis, fallback sur la règle par défaut du `productType` (campagne nationale santé ou prêt). Voir §6 ci-dessous pour la liste des IDs disponibles. |
...
## 6. Campagne / règle de routage (`metaRoutingRuleId`)

Chaque lead que vous créez doit être associé à une **règle de routage** côté CRMLeads. Cette règle détermine à quel pool de courtiers le lead est distribué (campagne nationale, campagne géolocalisée, exclusivité…).

L'**ID de la règle** est un identifiant CRMLeads (préfixé `cm…`, ex. `cmo8a4zla00xnmw20myzwv5yl`) que **l'admin CRMLeads vous transmet** lors de la mise en place. Si votre site couvre plusieurs cas (par ex. mutuelle santé + assurance de prêt, ou plusieurs zones géographiques), vous recevrez plusieurs IDs et devrez choisir celui qui correspond à chaque soumission.

### Comment ça fonctionne

| Cas | Champ `metaRoutingRuleId` | Effet |
|-----|---------------------------|-------|
| Vous fournissez un ID valide actif | renseigné | Lead routé vers le pool de courtiers de cette campagne |
| Vous omettez le champ | absent / `null` | Fallback sur la **règle par défaut du `productType`** (« Campagne Nationale » pour santé, « Assurance de prêt nationale » pour prêt) |
| Vous fournissez un ID inexistant ou désactivé | invalide | Le lead est créé mais ne sera **pas distribué** automatiquement (admin doit le traiter manuellement) |

### Recommandations

- **Si votre site capture un seul type de leads** (ex. mutuelle santé nationale uniquement) : l'admin vous donnera l'ID de la règle correspondante, vous le hardcodez en variable d'environnement `CRMLEADS_ROUTING_RULE_ID`.
- **Si votre site capture plusieurs types** : l'admin vous fournit un mapping `{ formId → metaRoutingRuleId }`, vous sélectionnez le bon en fonction du formulaire ou du contexte de soumission.
- **En cas de doute** : laissez le champ vide. Le fallback nationale est appliqué — pas idéal mais le lead est traité.

### Obtenir la liste à jour

L'admin peut consulter la liste depuis `/admin/meta-routing` (interface admin) et vous transmettre :
- `id` : l'identifiant CRMLeads à mettre dans `metaRoutingRuleId`
- `name` : libellé interne (ex. « Campagne Nationale », « Calais », « Voisin le Bretonneux »)

Ces IDs sont **stables** : ne changent pas tant qu'une règle n'est pas supprimée. Vous pouvez les hardcoder en config sans crainte.

### Cas multi-campagnes (un seul partenaire, plusieurs formulaires)

Un même compte partenaire (un seul couple `apiKey` / `secret`) peut créer des leads sur **autant de campagnes différentes que nécessaire**, en faisant simplement varier `metaRoutingRuleId` à chaque appel. Vous n'avez pas besoin d'un compte partenaire par campagne.

**Étape 1 — Configurer le mapping**

L'admin CRMLeads vous transmet un tableau de correspondance `{ contexte côté partenaire → metaRoutingRuleId CRMLeads }`. Exemple type :

| Contexte côté votre site | Libellé CRMLeads | `metaRoutingRuleId` |
|--------------------------|------------------|---------------------|
| `/mutuelle/calais` | Calais | `cmo10g5pw0004pa21jq1a1qsu` |
| `/mutuelle/voisin-le-bretonneux` | Voisin le Bretonneux | `cmnyq1l9q00cspa20oil3j5zi` |
| `/mutuelle/national` | Campagne Nationale | `cmo8a4zla00xnmw20myzwv5yl` |
| `/assurance-pret` | Assurance de prêt nationale | `cmohek97c0004mw20y6ow6hju` |

**Étape 2 — Stocker la table en config**

```javascript
// config/crmleads-routing.js
export const ROUTING_RULES = {
  'mutuelle-calais':              'cmo10g5pw0004pa21jq1a1qsu',
  'mutuelle-voisin-le-bretonneux': 'cmnyq1l9q00cspa20oil3j5zi',
  'mutuelle-national':            'cmo8a4zla00xnmw20myzwv5yl',
  'assurance-pret':               'cmohek97c0004mw20y6ow6hju',
};

export function resolveRoutingRule(formKey) {
  const id = ROUTING_RULES[formKey];
  if (!id) {
    // Fallback prudent : on laisse undefined → CRMLeads applique sa règle par défaut
    console.warn(`[crmleads] formulaire inconnu "${formKey}", routage par défaut`);
    return undefined;
  }
  return id;
}
...
**Étape 3 — Sélectionner dynamiquement à la soumission**

```javascript
import { resolveRoutingRule } from './config/crmleads-routing.js';

app.post('/api/submit/:formKey', async (req, res) => {
  const formKey = req.params.formKey;          // ex. "mutuelle-calais"
  const routingRuleId = resolveRoutingRule(formKey);

  const payload = {
    productType: formKey.startsWith('assurance-pret') ? 'PRET' : 'SANTE',
    externalSource: `monsite-${formKey}`,
    ...(routingRuleId && { metaRoutingRuleId: routingRuleId }),
    consentProof: { /* … */ },
    prospect: { /* … */ },
    // familyComposition obligatoire pour SANTE
    ...(formKey.startsWith('mutuelle-') && {
      familyComposition: { members: buildMembersFromForm(req.body) },
    }),
  };

  const result = await createLead(payload);
  res.json({ ok: true, isDuplicate: result.isDuplicate });
});
```

Avec cette approche, le partenaire :
- N'a qu'**un seul couple credentials** à gérer
- Sa table de routage est versionnée dans son code (Git) — facile à diffuser entre dev/staging/prod
- Le `productType` peut être déduit du contexte du formulaire (ex. tout formulaire `assurance-pret-*` → `PRET`)
- En cas de nouveau formulaire/campagne, l'admin CRMLeads transmet juste un nouvel ID à ajouter dans la table

> ⚠️ **Limite à connaître** : aucune restriction technique n'empêche le partenaire d'envoyer un `metaRoutingRuleId` qu'il n'est pas censé utiliser (le cloisonnement multi-tenant strict n'est pas implémenté en v1). Le contrat repose sur la confiance et la transmission contrôlée des IDs par l'admin.
...
## 9. Exemple complet (Node.js / Express)
...
const ROUTING_RULE_ID = process.env.CRMLEADS_ROUTING_RULE_ID;  // ID de la campagne CRMLeads (transmis par l'admin)
...
app.post('/api/submit-mutuelle-form', async (req, res) => {
  ...
  const payload = {
    productType: 'SANTE',
    externalSource: 'monsite-formulaire-mutuelle-v1',
    metaRoutingRuleId: ROUTING_RULE_ID,
    consentProof: {
...
## 11. Que se passe-t-il après la création ?
